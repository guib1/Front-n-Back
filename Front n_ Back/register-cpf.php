		...
		<div class="user_card">
			...
			<form method="POST" action="">
						...
						<div class="input-group mb-3">
							...
							<input type="number" name="cpf" id="bcpf" class="form-control output_user" value="" placeholder="CPF">
						</div>
...
<?php
if (!empty($_POST['nome']) && !empty($_POST['endereco']) && !empty($_POST['cpf']) && !empty($_POST['pwd']) && !empty($_POST['pwd2'])) {
	$name = $_POST['nome'];
	...
	$sql = "SELECT * FROM `tbusuarios` WHERE CPF_CNPJ='$cpf';";
	...
	if (isset($resultado)) {
		...
		if ($pwd == $pwd2) {
			$sql = "INSERT INTO tbusuarios (ID, NOME, ENDERECO, CPF_CNPJ, SENHA) VALUES ('','$name','$endereco',$cpf,'$pwd');";
			$resultado = mysqli_query($conn, $sql);
			echo "
		<span id='correctUser'
